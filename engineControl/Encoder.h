...
////////////////////////
//Чтобы работали библиотеки ардуино из более ранних версий
//arduino IDE
...
//прерывания объеденены в структуру
//менять не стоит перестаёт работать
typedef struct {
...
class Encoder //Создаём класс Encoder
{
public:
...
#endif
		//не работает, но и не столь важно
		}
...
private:
	static void update(Encoder_internal_state_t *arg) {
		...
		"L%=2:" "ldi	r30, lo8(pm(L%=table))"	"\n\t"
			...
			//перемещаем в другую статическую функцию
		"L%=table:"				"\n\t"
			"rjmp	L%=end"			"\n\t"	   // 0
			"rjmp	L%=plus1"		"\n\t"	   // 1
			"rjmp	L%=minus1"		"\n\t"	 // 2
...
#ifdef ENCODER_USE_INTERRUPTS
	// не поддерживает передачу указателя
...
#endif // Конец ыункции ENCODER_USE_INTERRUPTS
